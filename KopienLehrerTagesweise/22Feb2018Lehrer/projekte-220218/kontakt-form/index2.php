...
require_once './lib/contact-form1.conf.php';

$posted = isSent();

if ($posted) {
    /* 
        Schleife über die Konfiguration, alle in der Konfiguration
        angegebenen Einträge werden überprüft
    */
    foreach( $formConf as $fieldName => $value ) {
        // required prüfen
        if ( $value == true && required($fieldName) == false ) {
            echo "<p>Bitte das Feld $fieldName ausfüllen.</p>";
        }
    }
}
?>
